...
with open(f"{name}.m","w") as file:
    ...
    file.write(f'% Derivatives hitting the activation fct are identity matrices so we can disregard them\n')
    
    file.write(f'% Calculate the derivatives and update the weight matrices and biases\n')

    file.write(f"    DL = (f{i+1} - y)'; \n")

    file.write(f'    Db{i+1} = DL;\n')
    ...
    for i in range(len(layers)-1):
        file.write(f'    Db{len(layers)-i} = Db{len(layers)+1-i} * A{len(layers)+1-i};\n')
    ...
    file.write(f'    Db{len(layers)-i-1} = Db{len(layers)-i} * A{len(layers)-i};\n')
